<script>
    ...
    $(document).ready(function () {
        ...
        $('select[name="land_id"]').on('change', function () {
            ...
            if (landId) {
                $.ajax({
                    url: '{{ route("engineering.land.getLandDetails") }}',
                    type: 'GET',
                    data: {id: landId},
                    success: function (response) {
            ...
            if (investorId) {
                $.ajax({
                    url: '{{ route("engineering.getInvestorDetails") }}',
                    type: 'GET',
                    data: {id: investorId},
                    success: function (response) {
            ...
            var initAddproject = function () {
                    ...
                    form,
                    {
                        fields: {
                            total_price: {
                                validators: {
                                    notEmpty: {
                                        message: "{{ __('engineering.This field is required') }}"
                                    }
                                }
                            },
                            duration: {
                        ...
                        validator.validate().then(function (status) {
                            if (status == 'Valid') {
                                ...
                                setTimeout(function () {
                                    ...
                                    const url = `{{ route('contractors.projects.submit_quote',$project->id) }}`;
                ...
                cancelButton.addEventListener('click', e => {
                    ...
                    Swal.fire({
                        ...
                        if (result.value) {
                            window.location.href = "{{ route('contractors.projects.index') }}"; // Redirect to land.index route
...
// On document ready
        ...
        $(document).on('click', '.delete-attachment-btn', function() {
            ...
            Swal.fire({
                ...
                const url = `{{url("/")}}/attachments/${id}`; // Build the correct URL
                $.ajax({
                    url: url,
                    type: 'DELETE',
                    data: {
