...
class MyProjectsController extends Controller
{
    ...
    {
        return view('contractors.projects_management.my_projects.list');
    }
    ...
    public function getProjectsDetails(Request $request)
    {
        ...
        return view('contractors.projects_management.ajax.project_details', compact('projects'))->render();
    ...
    public function enter_units($id)
    {
        $awardingApprovedId = \App\Models\Lookups::where('master_key', 'project_status_cd')
            ->where('item_key', \App\Models\Projects::PROJECT_STATUS_CONTRACTOR_AWARDING_APPROVED)
            ->value('id');
        $unitsAddedId = \App\Models\Lookups::where('master_key', 'project_status_cd')
            ...
            ->value('id');
        $data['offers'] = ContractorOffers::query()
            ...
            ->with('project')
            ->orderBy('contractor_offers.id', 'desc')->get()
        ;
        ...
        $data['project_id'] = $id;
        return view('contractors.projects_management.my_projects.enter_units',$data);
    ...
    public function get_awardingApproved_offers(Request $request)
    {
        ...
        $offerApprovedId = \App\Models\Lookups::where('master_key', 'contractor_offer_status')
            ->where('item_key', \App\Models\ContractorOffers::STATUS_APPROVED)
            ->value('id');
    ...
    public function saveProjectUnits(Request $request)
    {
        ...
        if ($action === 'send_to_valuation') {
            ...
            $project_log->user_id = Auth::user()->id ;
